 ...
 */
public class HtmlTreeBuilder2 extends TreeBuilder {
    // tag searches
    ...
    private HtmlTreeBuilderState2 state; // the current state
    private HtmlTreeBuilderState2 originalState; // original / marked state
    ...
    Document parse(String input, String baseUri, ParseErrorList errors, ParseSettings settings) {
        state = HtmlTreeBuilderState2.Initial;
    ...
    boolean process(Token token, HtmlTreeBuilderState2 state) {
        currentToken = token;
        return state.process(token, this);
    }

    void transition(HtmlTreeBuilderState2 state) {
        this.state = state;
    }

    HtmlTreeBuilderState2 state() {
        return state;
    }
    ...
    HtmlTreeBuilderState2 originalState() {
    ...
    void error(HtmlTreeBuilderState2 state) {
    ...
    void resetInsertionMode() {
        ...
        for (int pos = stack.size() -1; pos >= 0; pos--) {
            ...
            if ("select".equals(name)) {
                transition(HtmlTreeBuilderState2.InSelect);
                break; // frag
            } else if (("td".equals(name) || "th".equals(name) && !last)) {
                transition(HtmlTreeBuilderState2.InCell);
                break;
            } else if ("tr".equals(name)) {
                transition(HtmlTreeBuilderState2.InRow);
                break;
            } else if ("tbody".equals(name) || "thead".equals(name) || "tfoot".equals(name)) {
                transition(HtmlTreeBuilderState2.InTableBody);
                break;
            } else if ("caption".equals(name)) {
                transition(HtmlTreeBuilderState2.InCaption);
                break;
            } else if ("colgroup".equals(name)) {
                transition(HtmlTreeBuilderState2.InColumnGroup);
                break; // frag
            } else if ("table".equals(name)) {
                transition(HtmlTreeBuilderState2.InTable);
                break;
            } else if ("head".equals(name)) {
                transition(HtmlTreeBuilderState2.InBody);
                break; // frag
            } else if ("body".equals(name)) {
                transition(HtmlTreeBuilderState2.InBody);
                break;
            } else if ("frameset".equals(name)) {
                transition(HtmlTreeBuilderState2.InFrameset);
                break; // frag
            } else if ("html".equals(name)) {
                transition(HtmlTreeBuilderState2.BeforeHead);
                break; // frag
            } else if (last) {
                transition(HtmlTreeBuilderState2.InBody);
                break; // frag
